...
class MeetingTypeRepository:
    def __init__(self, uow):
        self.uow = uow
        self.table = uow.tables.meetingtypes

    def get_all_meetingtypes(self, query_parameters):
        return self.uow.apply_query_parameters(self.table, query_parameters)

    def get(self, id):
        model_raw = self.uow.run(self.table.get(id))
        model = MeetingType(model_raw)
        return model
    ...
    def does_meeting_type_exist_by_name(self, name):
