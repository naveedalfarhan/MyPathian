...
class ActionItemTypeRepository:
    def __init__(self, uow):
        self.uow = uow
        self.table = uow.tables.action_item_types

    def get_all_actionitemtypes(self, query_parameters):
        return self.uow.apply_query_parameters(self.table, query_parameters)

    def get(self, id):
        model_raw = self.uow.run(self.table.get(id))
        model = ActionItemType(model_raw)
        return model
    ...
    def does_type_exist_by_name(self, name):
