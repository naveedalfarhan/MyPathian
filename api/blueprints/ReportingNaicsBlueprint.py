...
ReportingNaicsBlueprint = Blueprint("ReportingNaicsBlueprint", __name__)


@ReportingNaicsBlueprint.route("/api/ReportingNaics/GetIntensityData", methods=["POST"])
@login_required
...
def get_intensity_chart_data(naics_codes, account_type, report_year, comparison_type, demand_type, g):
    ...
    # get all accounts for naics code and it's descendants
    for nc in naics_codes:
        data_insert = []
        accounts = []
        naics_code = g.uow.naics.get_by_code(nc)
        group_descendants = g.uow.naics.get_group_descendants(nc)
        ...
        final_data.append({"name": naics_code["name"], "data": data_insert})
...
def get_total_energy_chart_data_web(naics_code, report_year, benchmark_year, account_type, comparison_type, demand_type,
                                    y_units, y_unit_map, g):
    """
    Gets the data for the energy consumption chart
    :param naics_code: naics code being reported on
    ...
    # get group descendants of naics code
...
def get_pf_chart_data_web(naics_code, report_year, benchmark_year, comparison_type, demand_type, g):
    """
    Gets and calculates all data needed for powerfactor charts
    :param naics_code: naics code to find data for
    ...
    naics_obj = g.uow.naics.get_by_code(naics_code)
    descendants = g.uow.naics.get_group_descendants(naics_code)
    accounts = []

    # get all accounts of group descendants of naics code
    for gr in descendants:
...
@ReportingNaicsBlueprint.route("/api/ReportingNaics/GetTotalEnergyData", methods=["POST"])
@login_required
@from_request_body("config", ReportingConfiguration)
@require_permissions(['Run NAICS Reports'])
...
@ReportingNaicsBlueprint.route("/api/ReportingNaics/GetGroups", methods=["GET"])
@login_required
@from_query_string("code", str, "code")
@require_any_permission(['Manage Groups', 'View Groups'])
def get_groups(code):
    groups = g.uow.naics.get_group_descendants(code)
...
def get_peak_report_data(config, g):
    ...
    final_data = []
    for naics_code in config.entity_ids:
        descendants = g.uow.naics.get_group_descendants(naics_code)
        naics_name = g.uow.naics.get_by_code(naics_code)["name"]
        accounts = []
        # get all accounts for naics code descendants
...
@ReportingNaicsBlueprint.route('/api/ReportingNaics/peak', methods=["POST"])
@from_request_body("config", ReportingConfiguration)
@login_required
@require_permissions(['Run NAICS Reports'])
...
def generate_consumption_report(config, y_units, y_unit_map, total_energy_chart_info, submitted_by_user, submitted_to, g):
    ...
    year_group_grouping = {}
    # loop through every NAICS code in the configuration and get their consumption chart data
    for code in config.entity_ids:
        # get the naics code and it's group descendants
        naics_code = g.uow.naics.get_by_code(code)
        group_descendants = g.uow.naics.get_group_descendants(code)

        # get all accounts for the NAICS code
        accounts = []
        ...
        data = SharedReporting.get_total_energy_chart_data_pdf(accounts, config.report_year, config.benchmark_year, config.demand_type,
                                               y_units, y_unit_map, g)
        total_energy_chart_info.data.append({'entity': naics_code['name'], 'data': data['chart_data']})
        ...
        for year in data['year_data']:
            ...
            if not year in year_group_grouping:
                ...
                year_group_grouping[year] = [{'entity': naics_code['name'] + ' (' + str(year) + ')',
                                              'data': data['year_data'][year]}]
            else:
                # append the entry
                year_group_grouping[year].append({'entity': naics_code['name'] + ' (' + str(year) + ')',
                                                  'data': data['year_data'][year]})
...
def generate_powerfactor_report(config, y_units, y_unit_map, total_energy_chart_info, submitted_by_user, submitted_to, g):
    ...
    for code in config.entity_ids:
        ...
        total_energy_chart_info.data.append({'entity': naics_code["name"], 'data': data})
...
def generate_kvarh_report(config, y_units, y_unit_map, total_energy_chart_info, submitted_by_user, submitted_to, g):
    ...
    for code in config.entity_ids:
        ...
        # get data associated with each account in the naics code
        accounts = []
        group_descendants = g.uow.naics.get_group_descendants(code)
        for gr in group_descendants:
            accounts += SharedReporting.get_accounts_for_group(gr, 'Electric', config.report_year, config.comparison_type, config.demand_type, g)
...
def generate_consumptiontext_report(config, submitted_by_user, g):
    ...
    :return: report path
    """
    report_path = DataReports.generate_consumptiontext_report(config.report_year, config.benchmark_year, SharedReporting.generate_consumptiontext_report(config, 'naics', g), submitted_by_user, config.account_type)
    return report_path
...
def generate_variance_report(config, y_units, y_unit_map, submitted_by_user, submitted_to, g):
    ...
    for code in config.entity_ids:
        ...
        data = {"entity_name": naics_code["name"], "sitedata": [
            {"label": "",
             "data": []},
            {"label": "",
             "data": []},
        ...
        data["entity_name"] = naics_code["name"]
...
@ReportingNaicsBlueprint.route("/api/ReportingNaics/GetReport", methods=["POST"])
@login_required
@from_request_body("config", ReportingConfiguration)
